...
VoxelDream 是一套用于制作、编辑和变换游戏资产与场景的离散空间 3D 创作系统。你可以从零开始，也可以从图片、Mesh 或已有体素内容出发，使用直接编辑工具继续精修，并将作品导出到 Minecraft、游戏引擎或标准 3D 工作流。
...
VoxelDream 在有限、整数对齐的三维工作空间中保留可继续编辑的体素源数据，因此可以针对空间占用、连通区域、建模工具范围、选区变换、参数化生成、紧凑存储和网格重建使用专用算法。体素在这里不只是一种视觉风格，而是一种可控的模型表示，让创建、编辑、保存和导出始终建立在同一份底层数据上。
